...
LITECOIND=${LITECOIND:-$SRCDIR/bixbcoind}
LITECOINCLI=${LITECOINCLI:-$SRCDIR/bixbcoin-cli}
LITECOINTX=${LITECOINTX:-$SRCDIR/bixbcoin-tx}
LITECOINQT=${LITECOINQT:-$SRCDIR/qt/bixbcoin-qt}

[ ! -x $LITECOIND ] && echo "$LITECOIND not found or not executable." && exit 1

# The autodetected version git tag can screw up manpage output a little bit
BXBVER=($($LITECOINCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
