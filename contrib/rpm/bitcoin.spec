...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 4565
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 5963
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 14565
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 15963
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 4565
	%{_sbindir}/semanage port -d -p tcp 5963
	%{_sbindir}/semanage port -d -p tcp 14565
	%{_sbindir}/semanage port -d -p tcp 15963
	for selinuxvariant in %{selinux_variants}; do
