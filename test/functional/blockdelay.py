...
#        raw_input("press enter to start..")
        try:
            ...
            print("  Node0 has: %d" %
                  self.nodes[0].getblockfinalityindex(blocks[0]))
            print("")
...
# Node(3): [0]->..->[104]
        ...
        for i in range(0, 5):
            # block height 105 -6 -7 -8 - 9 - 10
            blocks.extend(self.nodes[0].generate(1))
            bl.append(blocks[len(blocks)-1])
            print("%2d) %s" % ((i+105), bl[i]))
        self.sync_all()
        for i in range(5, 12):
            # block height 111-12-13-14-15-16
...
#   Node(3): [0]->..->[104]
        ...
        # block height 105 - 6 -7 -8 -9-10 -11 12 13 14
...
#        raw_input("press enter to join the netorks..")
        ...
        for i in range(0, 4):
            try:
                print("  Node%d sees:" % i)
                print("      finality: %d" %
                      self.nodes[i].getblockfinalityindex(first_main_blockhash))
                print("")
